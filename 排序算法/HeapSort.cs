...
namespace HeapSort {
    ...
    class Solution {
        ...
        /// 从最后一个分支节点开始（n / 2 - 1的节点）,依次将所有以分支节点为根的二叉树调整成堆，
        /// 即先将子树构建成堆，再将父树构建成堆。
        /// 当这个过程持续到根节点时，整个二叉树就调整成了堆，即建堆完成。
        /// 
        /// 如何调整堆：假设被调整的分支节点为A，它的左孩子为B，右孩子为C。
        /// 那么当A开始进行堆调整时，根据上面建堆的方式，以B和以C为根的二叉树都已经是堆了。
        /// 如果节点A的值大于B和C的值（以大顶堆为例），那么以A为根的二叉树已经是堆。
        /// 如果A节点的值小于B节点或C节点的值，那么节点A与值最大的那个孩子节点变换位置。
        /// 此时需要将A继续与和它交换的那个孩子的原来的两个孩子进行比较，
        /// 以此类推，直到节点A向下渗透到适当的位置为止。
        /// 
        /// 如果要从小到大排序，则使用大顶堆，如果要从大到小排序，则使用小顶堆。
        /// 原因是堆顶元素需要交换到序列尾部
        /// </summary>
